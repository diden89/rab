			...
			if (children != '') {
				strMenu += '<tr data-id="' + v.rm_id + '" data-parent="' + v.rm_parent_id + '">';
					...
					strMenu += '<td style="text-align:center;">';
						if(v.masg_id != '')
						{
							strMenu += '<input name="masg_id[]" type="hidden" value="'+v.masg_id+'">';
						}
...
function loadTreeMenuData(usg_id,ug_id) 
{
	$.ajax({
		url: siteUrl('settings/menu_access_sub_group/get_menu_data'),
		type: 'POST',
		dataType: 'JSON',
		data: {
		...
		success: function (result) {
			if (result.success) {
				$('.collaptable').find('tbody').html('');
				$('.collaptable').find('tbody').append('<input type="hidden" value="'+usg_id+'" name="usg_id">');
...
function loadData(data,callback) 
{
	...
	if(data != undefined)
	{
		for (var x in data) {
			...
			listGroup.append('<a class="list-group-item list-group-item-action" id="list-profile-list" data-toggle="list" href="#" aria-controls="profile" data-usg_id="' + newData.usg_id + '" data-ug_id="' + newData.usg_group + '">' + newData.usg_caption + '</a>');
		}		
	}
	else
	{
		listGroup.html('<p class="text-muted">User Sub Group Not Found!!</p>');
	}
	callback();
}
...
$(document).ready(function() {
	$.ajax({
		url: siteUrl('settings/menu_access_sub_group/get_user_group'),
		type: 'POST',
		dataType: 'JSON',
		data: {
	...
	$('#txt_group_id').on('change',function(e){
		...
		$.ajax({
			url: siteUrl('settings/menu_access_sub_group/get_user_sub_group'),
			type: 'POST',
			dataType: 'JSON',
			data: {
				...
				if (result.success) {
					loadData(result.data, function() {
						$('#listGroup a').on('click', function (e) {
							...
							usg_id = $(this).attr('data-usg_id');
							ug_id = $(this).attr('data-ug_id');
							loadTreeMenuData(usg_id,ug_id);
						});
					});
	...
	$('#addAccessGroup').submit(function(e){
		...
		$.ajax({
			url: siteUrl('settings/menu_access_sub_group/store_data'),
			type: 'POST',
			dataType: 'JSON',
			...
			success: function(result) {
				if (result.success) {
					...
					loadTreeMenuData(result.usg_id,result.ug_id);
