...
class Data_jurusan extends MY_Controller {
	function __construct()
	{
		...
		$this->load->model('data_jurusan_model','djm');
	...
	public function index($src = array())
	{
		...
		{	
	        ...
	        $config['base_url'] = base_url('data_jurusan/index'); //site url
	...
	{
		$data['nama_jurusan'] =$this->input->post('txt_nama_jurusan');
		$id =$this->input->post('txt_jurusan_id');

		if( ! empty($id))
		{
			$upd_jurusan = $this->djm->do_update($data,$id,'data_jurusan');

			echo json_encode(array(
	        	"status" => TRUE,
	        	"url" => base_url('data_jurusan')
	        ));
		}
		else
		{
			$inp_jurusan = $this->djm->do_upload($data,'data_jurusan');
	...
	public function delete()
	{
		...
		$get_before_update = $this->djm->get_data_any_table(array('id'=>$id),'data_jurusan')->row();
		$status = ($get_before_update->is_active == 'Y') ? 'N' : 'Y';
		$deleteemployee = $this->djm->delete(array('is_active' => $status),$id,'data_jurusan');
		 redirect(base_url('data_jurusan'), 'refresh');
	}

	public function delete_image()
	{
		$id = $this->input->post('id_berkas');
		$id_data_jurusan = $this->input->post('id_pendonor');
	    ...
	    $berkas = $this->djm->get_img_berkas(array('id_pendonor' => $id_data_jurusan));
	    ...
	    // redirect(base_url('data_jurusan/cu_action/edit/'.$id_data_jurusan), 'refresh');
	}
}