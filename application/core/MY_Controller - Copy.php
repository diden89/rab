		...
		if (count($params) > 0)
		{
			...
			if ($validate_login !== FALSE && $validate_login->num_rows() > 0)
			{
				...
				$this->session->set_userdata(array(
					'token' => $this->encryption->encrypt($row->ud_id.':'.$row->username),
		...
		foreach ($datas as $data)
		{
			if ($data->parent_id == $parent_id)
			{
				...
				if ($children)
				{
					if ($idx == 0)
					{
						$html_start = '<li class="dropdown nav-item"><a class="dropdown-toggle nav-link" href="'.$data->url.'" id="menuDropdown" data-toggle="dropdown">'.ucwords(strtolower($data->caption)).'<b class="caret"></b></a><ul class="dropdown-menu nav-item">';
						$html_end = '</ul></li>';
					}
					elseif ($idx > 0)
					{
						$html_start = '<div class="dropdown-submenu nav-item"><a class="dropdown-item nav-link" href="'.$data->url.'">'.ucwords(strtolower($data->caption)).'</a><ul>';
						$html_end = '</ul></div>';
					}
				...
				else
				{
					if ($idx == 0)
					{
						$html = '<li class="nav-item"><a href="'.$data->url.'" class="nav-link">'.ucwords(strtolower($data->caption)).'</a></li>';
					}
					elseif ($idx == 1)
					{
						$html = '<li class="nav-item"><a href="'.$data->url.'" class="nav-link">'.ucwords(strtolower($data->caption)).'</a></li>';
