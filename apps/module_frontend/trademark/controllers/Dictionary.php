 ...
 * @version 1.0
 * @access Public
 * @link /ahp_merekdagang_frontend/apps/module_frontend/trademark/controllers/Dictionary.php
 */

class Dictionary extends NOOBS_Controller
{
	function __construct()
	{
		parent::__construct();
		$this->load->model('trademark/dictionary_model', 'db_dictionary');
	}

	public function index()
	{
		$this->store_params['header_title'] = 'Dictionary';
		$this->store_params['breadcrumb'] = array(
			array('', 'Home'),
			array('trademark/dictionary', 'Dictionary')
		);

		$this->store_params['source_bot'] = array(
			'<script src="'.base_url('scripts/trademark/dictionary.js').'"></script>'
		);

		$this->view('dictionary_view');
	}
	...
	public function get_data()
	{
		...
		{
			$get_data = $this->db_dictionary->get_data($post);
	...
	public function get_autocomplete_data()
	{
		...
		{
			$get_autocomplete_data = $this->db_dictionary->get_autocomplete_data($post);
	...
	public function get_data_similar_word()
	{
		...
		{
			$get_data_similar_word = $this->db_dictionary->get_data_similar_word($post);
	...
	public function popup_modal()
	{
		...
		if (isset($post['action']) && ! empty($post['action']) && $post['action'] == 'popup_modal')
		{
			...
			$this->_view('dictionary_popup_modal_view', $post);
		}
		else $this->show_404();
	}
			...
			if ($txt_new_similar_word_id)
			{
				...
				if (isset($post['txt_d_id']) && !empty($post['txt_d_id'])) {
					...
					{ 
						...
						$success = $this->_edit_dictionary($txt_new_d_id, array_unique($new_arr_similar_word));
					}
	...
	public function edit_data()
	{
		...
		if (isset($post['action']) && ! empty($post['action']) && $post['action'] == 'edit_data')
		{
			...
			$edit_data = $this->db_dictionary->edit_data($post);
	...
	public function delete_data()
	{
		...
		{
			$delete_data = $this->db_dictionary->delete_data($post);
			...
			if ($delete_data)
			{
				...
				for ($i=0; $i < count($arr_similar_word) ; $i++) 
				{
					...
					$success = $this->_edit_dictionary($new_d_id, array_unique($arr_similar_word_by_d_id));
				}
	...
	{
		$check_word_available = $this->db_dictionary->check_word_available($word);

		if ($check_word_available->num_rows() > 0) exit(json_encode(array('success' => FALSE, 'msg' => 'This word already exist in dictionary')));
		else return TRUE;
	}

	private function _store_new_word($word)
	{
		return $this->db_dictionary->store_new_word($word);
	}

	private function _edit_dictionary($txt_d_id, $arr_similar_word)
	{
		return $this->db_dictionary->edit_dictionary($txt_d_id, $arr_similar_word);
	}

	private function _check_arr_similar_word_by_d_id($txt_d_id)
	{
		$arr_similar_word_by_d_id = $this->db_dictionary->check_arr_similar_word_by_d_id($txt_d_id);
